...
-- | FILE     : Undo_Diag.sql                                                   |
-- | CLASS    : Undo Segments                                                   |
-- | PURPOSE  : The script can be used to collect the necessary diagnostic 		  |	
-- |            information of the Automatic Undo Management, run as sysdba     |
-- | NOTE     : This script is provided by Maclean Liu                          |
-- +----------------------------------------------------------------------------+
...
PROMPT 
PROMPT +------------------------------------------------------------------------+
PROMPT | Report   : Undo_Diag                                                   |
PROMPT | Instance : &current_instance                                           |
PROMPT | USER     : &user_name							 |
PROMPT +------------------------------------------------------------------------+


PROMPT specify log path as parameter path:
DEFINE log_path = &path
DEFINE spool_file = &log_path/Undo_Diag.out
SPOOL &spool_file
...
group by rollup (TABLESPACE_NAME,status)
order by 1,2
/
